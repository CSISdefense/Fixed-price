...
* UnmodifiedIsSomeCompetition is the classification given by the first record for the contract (`r sprintf("%1.2f%%",nrow(subset(ContractSample,UnmodifiedIsSomeCompetition=="Unlabeled")) /nrow(ContractSample)*100)` missing data).
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(Ceil)&UnmodifiedIsSomeCompetition=="Comp."),
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(Ceil)&!is.na(IDV)&UnmodifiedIsSomeCompetition=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(Ceil)&!is.na(SimpleArea)&UnmodifiedIsSomeCompetition=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
    geom_bar(binwidth=1)+scale_x_continuous(limits=c(0,50))+facet_grid(Ceil~SimpleArea,scales="free_y")+
    theme(strip.text.y=element_text(angle=0))#size=axis.text.size,



ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&!is.na(SimpleArea)&UnmodifiedIsSomeCompetition=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
    geom_bar(binwidth=1)+scale_x_continuous(limits=c(0,50))+facet_grid(FxCb~SimpleArea,scales="free_y")+
    theme(strip.text.y=element_text(angle=0))#size=axis.text.size,



ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&SimpleArea=="R&D"&UnmodifiedIsSomeCompetition=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
    geom_bar(binwidth=1)+scale_x_continuous(limits=c(0,50))+facet_grid(FxCb~SimpleArea,scales="free_y")+
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&!is.na(IDV)&SimpleArea=="Products"&UnmodifiedIsSomeCompetition=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
...
IDVBox<-BoxplotWrapper(
    "Box plot of number of offers received by ceiling and IDV",
    "Box plot of number of offers received by ceiling and IDV",
    "Initial contract ceiling",
    "Number of Offers Received" ,
    Coloration,
    subset(CompleteModelAndDetail,!is.na(IDV)&!is.na(Ceil)),
"IDV",    
    "UnmodifiedNumberOfOffersReceived",
)
IDVBox<-IDVBox+coord_cartesian(ylim=c(0,30))+
    facet_wrap(~Ceil,ncol=5)+
    theme(axis.text.x=element_text(angle=0))#size=axis.text.size,

mean(subset(CompleteModelAndDetail,
            IDV=="IDV"&Ceil=="[30m+]")$UnmodifiedNumberOfOffersReceived,,na.rm=TRUE)
...
OffersByCeil<-HistogramOrDensityWrapper(
    ...
    ,"Frequency"
    ,Coloration
    ,subset(CompleteModelAndDetail,!is.na(Ceil)&UnmodifiedIsSomeCompetition=="Comp.")
    ,"histogram"
    ,"UnmodifiedNumberOfOffersReceived"
    ,"Ceil"
