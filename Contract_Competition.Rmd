...
* Comp is the classification given by the first record for the contract (`r sprintf("%1.2f%%",nrow(subset(ContractSample,Comp=="Unlabeled")) /nrow(ContractSample)*100)` missing data).
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(Ceil)&!is.na(SimpleArea)&Comp=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&!is.na(SimpleArea)&Comp=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&SimpleArea=="R&D"&Comp=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
    geom_bar(binwidth=1)+scale_x_continuous(limits=c(0,50))+facet_grid(FxCb~SimpleArea,scales="free_y")+
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&!is.na(IDV)&SimpleArea=="Products"&Comp=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&!is.na(IDV)&!is.na(Ceil)),
    aes_string(x = "FxCb",fill="FxCb"),
    main = "Distribution by duration and ceiling"
    ) +
    geom_bar()+facet_grid(IDV~Ceil,scales="free_y")+
    theme(strip.text.y=element_text(angle=0))#size=axis.text.size,

ggplot(
    data = subset(CompleteModelAndDetail,!is.na(FxCb)&!is.na(IDV)&!is.na(Ceil)),
    aes_string(x = "IDV",fill="IDV"),
    main = "Distribution by duration and ceiling"
    ) +
...
OffersByCeil<-HistogramOrDensityWrapper(
    ...
    ,"Frequency"
    ,Coloration
    ,subset(CompleteModelAndDetail,!is.na(Ceil)&Comp=="Comp.")
    ,"histogram"
    ,"UnmodifiedNumberOfOffersReceived"
    ,"Ceil"
