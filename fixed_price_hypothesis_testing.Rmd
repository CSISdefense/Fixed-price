...
# undebug(TerminationHypothesisTester)
TermSizeDF<-TerminationHypothesisTester(compGin)
#No redundant tests to remove

# color=Control,shape=Control
ggplot(TermSizeDF,
...
# undebug(TerminationHypothesisTester)
TermAircraftDF<-TerminationHypothesisTester(AircraftFind,"Aircraft")
TermAircraftDF<-rbind(TermAircraftDF,
      TerminationHypothesisTester(NotAircraftFind,"Not Aircraft")
      )
#Remove redundant tests
TermAircraftDF<-subset(TermAircraftDF,
                                    !CrossTab %in% unique(TermAircraftDF$Control))

ggplot(TermAircraftDF,
...
TermVehDF<-TerminationHypothesisTester(IDVFind,"IDV")
TermVehDF<-rbind(TermVehDF,
      TerminationHypothesisTester(NotIDVFind,"Award")
      )
#Remove redundant tests
TermVehDF<-subset(TermVehDF,
                                    !CrossTab %in% unique(TermVehDF$Control))
...
# undebug(TerminationHypothesisTester)
TermLongDurDF<-TerminationHypothesisTester(LongDurFind,"Long Dur.")
TermLongDurDF<-rbind(TermLongDurDF,
      TerminationHypothesisTester(NotLongDurFind,"Not Long Dur.")
      )
#Remove redundant tests
TermLongDurDF<-subset(TermLongDurDF,
                                    !CrossTab %in% unique(TermLongDurDF$Control))
ggplot(TermLongDurDF,
       aes(x=CrossTab,color=Control,shape=Control,y=FixedCostMargin)
       )+
    geom_point()+
    facet_grid(dVariable~iVariable)+ coord_flip()
...
# undebug(TerminationHypothesisTester)
TermCompDF<-TerminationHypothesisTester(CompFind,"Comp.")
TermCompDF<-rbind(TermCompDF,
      TerminationHypothesisTester(NotCompFind,"Not Comp.")
      )
#Remove redundant tests
TermCompDF<-subset(TermCompDF,
                                    !CrossTab %in% unique(TermCompDF$Control))
ggplot(TermCompDF,
       aes(x=CrossTab,color=Control,shape=Control,y=FixedCostMargin)
       )+
    geom_point()+
...
getEvidence(NotCompDF)


CompDF$Control<-"Comp"
NotCompDF$Control<-"Not Comp"
BothCompDF<-rbind(CompDF,NotCompDF)
