...
TermDF<-querygrain(AircraftFind,nodes=c("FxCb","Ceil","Term"), type="joint",result="data.frame")
TermDF<-ddply(TermDF,.(FxCb,Ceil),transform,p=Freq/sum(Freq))
# querygrain(AircraftFind, nodes=c("Offr","NChg","CRai","Term"), type="marginal")
TermDF<-dcast(TermDF,Term + Ceil ~ FxCb, sum, value.var="p")
TermDF$FixedCostMargin<-TermDF[,"Fixed-Price"]/TermDF[,"Cost-Based"]
TermDF$FixedCombMargin<-TermDF[,"Fixed-Price"]/TermDF[,"Combination \nor Other"]
colnames(TermDF)[colnames(TermDF)=="Term"]<-"Variable"

expNChgDF<-querygrain(AircraftFind,nodes=c("FxCb","Ceil","NChg"), type="joint",result="data.frame")
expNChgDF<-ddply(expNChgDF,.(FxCb,"Ceil),transform,p=Freq/sum(Freq))
...
expNChgDF<-ddply(expNChgDF,.(FxCb),summarise,expNChg=sum(expNChg))
expNChgDF<-dcast(expNChgDF, . ~ FxCb, sum, value.var="expNChg")
expNChgDF$Variable<-"Expected Number of Changes"
...
expNChgDF<-subset(expNChgDF,select=-c(.))

expCRaiDF<-querygrain(AircraftFind,nodes=c("FxCb","CRai"), type="joint",result="data.frame")
expCRaiDF<-ddply(expCRaiDF,.(FxCb),transform,p=Freq/sum(Freq))
...
expCRaiDF<-ddply(expCRaiDF,.(FxCb),summarise,expCRai=sum(expCRai))
expCRaiDF<-dcast(expCRaiDF, . ~ FxCb, sum, value.var="expCRai")
expCRaiDF$Variable<-"Ceiling Raising Change Orders %"
...
expCRaiDF<-subset(expCRaiDF,select=-c(.))


outcomes<-rbind(TermDF,expNChgDF,expCRaiDF)
