...
ggplot(
    data = subset(CompleteModelAndDetail,!is.na(Ceil)&!is.na(Veh)&Comp=="Comp."),
    aes_string(x = "UnmodifiedNumberOfOffersReceived",fill="Offr"),
    main = "Distribution by is full and open"
    ) +
    geom_bar(binwidth=1)+scale_x_continuous(limits=c(0,50))+facet_grid(Ceil~Veh,scales="free_y")+
