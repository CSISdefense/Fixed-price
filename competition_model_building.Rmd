...
cad.gin <- grain(as.graphNEL(CompetitionNetwork), data=ContractModel, smooth = 0.0000001)
summary(cad.gin)


querygrain(cad.gin, nodes = c("Offr"))

cpquery(CompetitionFitted, 
        event =  (Offr=="5+"),
        evidence = (FxCb=="Fixed-Price") & (Comp=="Comp."))

gin1.find <- setFinding(cad.gin, nodes = c("Comp","FxCb"),
...
ceiltest<-sapply(levels(ContractModel$Ceil),
                 function(x)  setFinding(cad.gin, nodes = c("Comp","FxCb","Ceil"),
                        states = c( "Comp.","Fixed-Price",x)))
