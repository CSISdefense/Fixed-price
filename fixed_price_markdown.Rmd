...
TotalOfObligated<-sum(contract.sample$SumofObligatedAmount)
CountOfContracts<-nrow(contract.sample)
double.density<-cbind(contract.sample,weight=1/CountOfContracts,weighting="Contract")
double.density<-rbind(double.density
                      ,cbind(contract.sample
                             ,weight=contract.sample$SumofObligatedAmount/TotalOfObligated
                             ,weighting="Obligation")
                      )
...
#         geom_bar(binwidth=0.05)+
        ...
        , scales="free_y"
                    , space="free_y")+scale_y_continuous(expand=c(0,10))+
...
#         facet_grid( UnmodifiedRnD1to5 ~ .
#         , scales="free_y"
#                     , space="free_y")+scale_y_continuous(expand=c(0,1))


# ggplot(
#         data=double.density
#         ,aes_string(x="pRnD1to5", weight="weight", fill="weighting")
#         ,main="R&D(1-5) distribution by contract count"
#         )+
# #         geom_bar(binwidth=0.05)+
#         geom_bar(alpha=0.2,adjust=0.5)+
#         facet_grid( UnmodifiedRnD1to5 ~ .
#         , scales="free_y"
#                     , space="free_y")+scale_y_continuous(expand=c(0,1))

# 
# density.comparison+ggplot(
#         data=contract.sample
...
contract.sample$isRnD1to5byStart<-factor(contract.sample$isRnD1to5byStart,levels=c(TRUE,FALSE),labels=c("R&D1-5 by Eff. Date","No R&D1-5 by Eff. Date"))
ggplot(
        data=contract.sample
        ,aes_string(x="pRnD1to5")
